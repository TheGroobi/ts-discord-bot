...
const readyCommands = (c: Client): boolean | void => {
...
const deployCommands = async (): Promise<void> => {
	...
	try {
		console.log(
			`Started refreshing, ${commands.length} application (/) commmands`
		);
		...
		console.log(
			...
			} application application (/) commmands`
		);
	} catch (e: unknown) {
		console.error(e);
	}
...
export { deployCommands, readyCommands };
